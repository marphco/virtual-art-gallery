...
const Favorites = () => {
  const { loading, error, data, refetch } = useQuery(GET_FAVORITES, {
    variables: { username: "levan" },
  });
  const [removeArt] = useMutation(REMOVE_ART, {
    refetchQueries: [
      { query: GET_FAVORITES, variables: { username: "levan" } },
    ],
  });
  ...
  const favorites = data.user.savedArt;
  ...
  const handleRemoveArt = async (artId) => {
    try {
      ...
      // After removing the artwork, manually refetch the favorites data
      refetch();
    } catch (error) {
    ...
    <div>
      ...
      <ul>
        {favorites.map((art) => (
          <li
            key={art.id}
            className="p-2 border border-gray-300 rounded-md shadow-sm m-2"
          >
            <div className="mb-2">
              {art.artist_titles.map((artist, index) => (
                <span
                  key={index}
                  className="inline-block bg-gray-200 text-gray-700 px-2 py-1 rounded-md mr-2 mb-1"
                >
                  {artist}
                </span>
              ))}
